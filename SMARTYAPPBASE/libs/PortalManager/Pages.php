...
class Pages extends \Controller
{
	...
	private function install_db()
	{
		...
		$created = (int)$this->db->query("SELECT 1 FROM information_schema.TABLES WHERE TABLE_SCHEMA = '".DB_NAME."' and TABLE_NAME = '".self::DB_TABLE."'")->fetchColumn();

		if( $created === 0 )
		{
			$qry = array();
			$qry[] = "CREATE TABLE IF NOT EXISTS `".self::DB_TABLE."`(
			  `ID` int(11) NOT NULL,
			...
			$qry[] = "ALTER TABLE `".self::DB_TABLE."`
			  ADD PRIMARY KEY (`ID`),
			  ADD UNIQUE KEY `eleres` (`eleres`),
			  ADD KEY `lathato` (`lathato`),
			  ADD KEY `szulo_id` (`szulo_id`),
			  ADD KEY `gyujto` (`gyujto`);";
